...
const columns = [
  ...
  },
  {
    ...
    header: 'Current Call Duration',
    meta: {
      headerProps: { className: 'text-900' }
    },
    cell: ({ row: { original } }) => {
      const { callDuration } = original;
  ...
  },
  {
    ...
    cell: ({ row: { original } }) => {
      const { queueLength } = original;
      return <span>{queueLength}</span>;  
    }
  },
  {
    ...
    cell: ({ row: { original } }) => {
      const { activeTickets } = original;
      return <span>{activeTickets}</span>;  
    }
  },
  {
    accessorKey: 'Action',
